...
export default function ExploreSpaces() {
  ...
  const getBounties = async () => {
    const res = await orbis.getPosts({
      context:
        "kjzl6cwe1jw14bfjlshsudnd99ozw3zz7p2buayz9x8lmyc5blj6wiiuifcday1",
      tag: "spatialDaoAllBounties",
    });
    setData(res.data);
  };
  const logout = async () => {
    ...
    renderBounties = data?.map((bounty: any, index: number) => {
      ...
      return (
        <Grid.Col key={index} lg={4} md={6}>
          <BountyCard
            query={bounty?.content?.tags[6]?.title}
            daoAddress={bounty.content.tags[5].title}
            streamId={bounty.streamId}
            title={bounty.content.tags[1].title}
            description={bounty.content.body}
